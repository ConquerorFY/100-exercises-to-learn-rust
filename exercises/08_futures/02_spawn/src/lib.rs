...
pub async fn echoes(first: TcpListener, second: TcpListener) -> Result<(), anyhow::Error> {
    let handle1 = tokio::spawn(async move {
        loop {
            let (mut socket1, _) = first.accept().await.unwrap();
            let (mut reader1, mut writer1) = socket1.split();
            tokio::io::copy(&mut reader1, &mut writer1).await.unwrap();
        }
    });
    let handle2 = tokio::spawn(async move {
        loop {
            let (mut socket2, _) = second.accept().await.unwrap();
            let (mut reader2, mut writer2) = socket2.split();
            tokio::io::copy(&mut reader2, &mut writer2).await.unwrap();
        }
    });

    if let Err(e1) = handle1.await {
        return Err(anyhow::Error::new(e1));
    }
    if let Err(e2) = handle2.await {
        return Err(anyhow::Error::new(e2));
    }
    Ok(())
}
